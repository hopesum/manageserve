...
router.post("/upload", upload.single("file"), (req, res) => {
  ...
  res.send({ code: 0, msg: "上传成功!", imgUrl: '/upload/shop/' + filename });
});
...
router.get("/info", (req, res) => {
  ...
  conn.query(sql, (err, data) => {
    ...
    data[0].avatar = "/upload/shop/" + data[0].avatar
...
router.post("/edit", (req, res) => {
  ...
  if (!Array.isArray(pics) || !Array.isArray(date) || !Array.isArray(supports)) {
    res.send({ code: 5001, msg: "参数错误!" });
    return;
  }
